 ...
 * return the `template.copy` receipt. Demo asset refs are stripped, then any
 * configured integration template transforms are applied to the written source
 * (a pure output-layer — the on-disk template is untouched); applied packages
 * are reported via `transformedBy`.
