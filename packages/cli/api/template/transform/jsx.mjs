 ...
 * sibling `function Name` or `const Name = () => …`. Returns a jscodeshift path
 * (so callers can identify returns that belong to it) or null. Anything else —
 * `export default forwardRef(…)`, `memo(Fn)`, a class, `export { X as default }`
 * — resolves to null, and the caller safely leaves the source untouched.
 *
 * @param {import('../../../authoring/codemod/type').JscodeshiftFactory} j
...
function findDefaultExportFunctionPath(j, root) {
  ...
  if (decl.type === 'Identifier') {
    ...
    const fnDecl = root.find(j.FunctionDeclaration, {id: {name}});
 ...
 */
export function wrapDefaultExportReturn(j, root, wraps) {
  ...
  if (fnNode.body && fnNode.body.type !== 'BlockStatement') {
    if (isElementNamed(fnNode.body, outermost)) return false;
  ...
  root.find(j.ReturnStatement).forEach((/** @type {any} */ rp) => {
    ...
    if (isElementNamed(arg, outermost)) return;
