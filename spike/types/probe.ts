...
defineTheme(
  ...
  {
    mobile: {
      // @ts-expect-error — `spacing` is not an axis of a theme
...
defineTheme(
  {name: 'bad-token'},
  {
    // @ts-expect-error — `--spacing-nope` is not a TokenName
...
defineTheme(
  {name: 'bad-value'},
  {
    // @ts-expect-error — a token value is a string or [light, dark] tuple
...
defineTheme({
  name: 'closed-root',
  // @ts-expect-error — `mobil` is not a theme axis
